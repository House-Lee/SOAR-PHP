 ...
 * @author Nikolai Kordulla , Modified by House.Lee
 */
class Parser
{
     ...
     */
    public function parse($protofile , $out_path = "./")
    {
        ...
        array_pop($name);
        $name = join($name, '.');
        $this->_create_class_file(rtrim($out_path , "/") .'/' . $name . '.pb.php');
    }
    ...
    private function _create_class_file($filename)
    {
        ...
        $header_require = "require_once (dirname(dirname(__FILE__)).\"/message/pb_message.php\");";
...
$out_path = dirname( __FILE__ )."/compiled/";
$in_file = $argv[1];
if(isset($argv[2]))
	$out_path = $argv[2];
$pb = new Parser();
$pb->parse($in_file , $out_path);
